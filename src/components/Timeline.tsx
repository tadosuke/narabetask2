...
export const Timeline = ({ tasks, workingHours, onTaskClick, onTaskDrop }: TimelineProps) => {
  // 時間をHH:MM形式から分単位に変換
  ...
  const getTaskAtPosition = (row: number, timeSlot: string): Task | null => {
    return timelineTasks.find(task => {
  ...
  return (
    <div className="timeline">
      <h3>タイムライン ({workingHours.start} - {workingHours.end})</h3>
          ...
          <div key={rowIndex} className="timeline-row">
            ...
            {timeSlots.map(timeSlot => {
              ...
              return (
                ...
                >
                  {taskAtPosition && timeSlot === taskAtPosition.position!.startTime && (
                    <TaskCard 
                      task={taskAtPosition}
                      onClick={onTaskClick}
                    />
